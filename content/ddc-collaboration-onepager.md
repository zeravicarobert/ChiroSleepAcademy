# When a Dentist and Chiropractor Meet the Same Sleep Apnea Patient
### Who Leads, Who Supports, and Why Sequence Matters

**Robert-authored, saved 2026-07-16.** A DDS↔DC collaboration one-pager. **Clean and
red-line safe** — respectful of dentistry and oral appliances, no cure claims,
no disparagement. Aligns with the white paper, the credible-edition **sequential
care model**, and "collaborate without collapsing scope." Source PDF:
`content/assets/DDC-collaboration-onepager.pdf`. Home: **Provider Network** pillar.

---

## Introduction — The New Kid in the Sandbox
- Sleep apnea patients are entering dental and chiropractic offices more than ever.
- Dentists and Doctors of Chiropractic often "see" the same patient at the same time.
- Without a shared framework, care becomes fragmented.

This explains **how coordination works best**, and **why sequence matters.**

## Section 1 — Why Dentists Often Open the Door First
Dentists already screen airway; CBCT provides visual authority; the first apnea
conversation feels natural in a dental setting. **This is not about ownership — it's
about patient acceptance.**

## Section 2 — Why CBCT Alone Is Not Enough
CBCT may show airway narrowing, jaw position, craniofacial anatomy. It does **not**
show spinal-curve load, pelvic imbalance, respiratory mechanics, postural
compensation, foot–spine relationships. **A narrow airway is a local finding; sleep
apnea is a global condition.**

## Section 3 — The Doctor of Chiropractic as Biomechanical Integrator
A post-graduate-trained Doctor of Chiropractic evaluates posture (CBP), pelvis & SIJ
(SOT/AK), cranial mechanics (BNS/CFR), spinal loading, and foot–pelvis–spine
integration — answering one question: *Will an oral appliance help — or create new
problems — without correcting global mechanics?*

## Section 3.5 — Where the Lanes Meet: the TMJ (and the SIJ that finishes it)
The **TMJ is the cross-over joint** — both licenses cover it, so it's the natural
front door for working together. And in the chiropractic biomechanical model, the
**TMJ and the sacroiliac joint (SIJ) behave as a reciprocal pair** — top and bottom
of the spine. So **TMJ work without SIJ focus is incomplete.** The dentist works the
TMJ; the Doctor of Chiropractic owns the SIJ (SOT/AK) — its paired partner.

Because a **California** Doctor of Chiropractic's body-parts scope is broader than the
dentist's (whole skeleton vs. mouth/jaw), the DC serves as the **case manager of the
whole-body sleep case.** In California the Doctor of Chiropractic **diagnoses and
manages** sleep apnea, snoring, and insomnia within full scope — and coordinates the
dentist on the appliance, and an MD/ENT when a case calls for it, as **peers.**
*(Scope described here is California-specific.)*

## Section 4 — The Proper Role of the Oral Appliance
Oral appliances can support airway patency and reduce collapse — and must be chosen
carefully. Selection is a **co-decision:** the DDS evaluates dental tolerance; the DC
evaluates biomechanical consequences. Both providers should be trained on the device.

## Section 5 — The Ideal Sequence (clean, ethical, effective)
1. DDS identifies airway concern.
2. DDS refers to DC for whole-system evaluation.
3. DC evaluates biomechanics.
4. DDS and DC select appliance together.
5. Appliance delivered.
6. DC reassesses global mechanics.
7. Adjustments and supports added as needed.

**No turf war. No duplication. No guessing.**

## Section 6 — Why This Protects Everyone
Dentists reduce chair time and may reduce liability exposure through documented
co-management; Doctors of Chiropractic practice within scope; manufacturers stay in
their lane; patients get better outcomes.

## Conclusion — The Playground Has Rules
> When everyone knows the rules of the playground, the new kid doesn't get hurt —
> and everyone wins.

## ⭐ Final thought (keeper)
> **Dentists open the door. Doctors of Chiropractic assess the whole house. Oral
> appliances support the structure — they don't replace it.**

---

*Editor's note: clean and strong. Only soften "reduce liability" to "may reduce
liability exposure through documented co-management" and let an attorney bless any
liability wording. Otherwise ready. Pairs with the DDS-facing "Chair-Hour
Efficiency" workbook (`content/dental-workbook-intro.md`).*
